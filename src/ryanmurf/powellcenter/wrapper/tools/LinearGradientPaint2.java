...
public class LinearGradientPaint2 implements Paint {
	...
	public LinearGradientPaint2(java.awt.geom.Point2D start,
			java.awt.geom.Point2D end, float[] fractions,
			java.awt.Color[] colors) {
		GRADIENT = new java.awt.LinearGradientPaint(start, end, fractions,
				colors, java.awt.MultipleGradientPaint.CycleMethod.NO_CYCLE);
		copyArrays(fractions, colors);
	}
	...
	public LinearGradientPaint2(java.awt.geom.Point2D start,
		...
		GRADIENT = new java.awt.LinearGradientPaint(start, end, fractions,
				colors, cycleMethod, colorSpace, gradientTransform);
		copyArrays(fractions, colors);
	}

	/**
	 * Returns the point where the wrapped java.awt.LinearGradientPaint will
	 * start
	 * 
	 * @return the point where the wrapped java.awt.LinearGradientPaint will
	 *         start
	 */
	...
	/**
	 * Returns the color that is defined by the given fraction in the linear
	 * gradient paint
	 * 
	 * @param FRACTION
	 * @return the color that is defined by the given fraction in the linear
	 *         gradient paint
	 */
	public java.awt.Color getColorAt(final float FRACTION) {
		...
		float interpolationFraction = (fraction - lowerLimit)
				/ (upperLimit - lowerLimit);

		return interpolateColor(colors[lowerIndex], colors[upperIndex],
	...
	public java.awt.PaintContext createContext(
			...
			java.awt.geom.AffineTransform X_FORM, java.awt.RenderingHints HINTS) {
		return GRADIENT.createContext(COLOR_MODEL, DEVICE_BOUNDS, USER_BOUNDS,
				X_FORM, HINTS);
	}
	 ...
	 * @param colors
	 */
	private void copyArrays(final float[] fractions,
	 ...
	 * 
	 * @param COLOR1
	 *            The first color as integer in the hex format 0xALPHA RED GREEN
	 *            BLUE, e.g. 0xFF00FF00 for a pure green
	 * @param COLOR2
	 *            The second color as integer in the hex format 0xALPHA RED
	 *            GREEN BLUE e.g. 0xFFFF0000 for a pure red
	 * @param FRACTION
	 *            The fraction between those two colors that we would like to
	 *            get e.g. 0.5f will result in the color 0xFF808000
	 * @return the interpolated color between color1 and color2 calculated by
	 *         the given fraction and interpolation
	 */
	private java.awt.Color interpolateColor(final java.awt.Color COLOR1,
			final java.awt.Color COLOR2, final float FRACTION) {
